{extends file="{$parent_template_path}/productdetails/config_options.tpl"}
...
{* WSC w100 anhängen START *}
                <div class="cfg-group js-cfg-group {if $configGroup@first}visited{/if} w100"
                    ...
                    {/block}
                    {block name='productdetails-config-container-group-description'}
{* WSC umfangreicher Umbau START *}
                    <div class="row">
                        {col cols=12 md=4 lg=2 order=1 order-lg=0}
                            <div class="group-description">
                                ...
                                {if $configLocalization->hatBeschreibung()}
                                    <blockquote class="blockquote">
                                      <p class="mb-0">{$configLocalization->getName()}</p>
                                      <footer class="blockquote-footer">{$configLocalization->getBeschreibung()}</footer>
                                    </blockquote>
                                {/if}
                              </div>
                          {/col}
{* WSC umfangreicher Umbau ENDE *}
                    {/block}

                    {block name='productdetails-config-container-group-items'}
{* WSC eine COL einfügen START *}
                        {col cols=12 md=8 lg=10 order=0 order-lg=1}
...
{* WSC LG auf 2 gesetzt Start *}
                                    {col cols=6 md=4 lg=2}
{* WSC LG auf 2 gesetzt ENDE *}
                                        {$bSelectable = 0}
                                        {if $oItem->isInStock()}
                                            {$bSelectable = 1}
                                        {/if}
...
{* START - Einblendung des Bildes als POPOVER *}
    <button type="button" class="btn btn-outline-dark btn-light btn-sm varLupe" data-toggle="popover" data-trigger="focus" title="{$oItem->getName()}" data-content="<img src={$oItem->getArtikel()->Bilder[0]->cPfadGross} class=img-fluid />">
        <i class="fas fa-search"></i>
    </button>
{* ENDE - Einblendung des Bildes als POPOVER *}
...
{* Preis entfernen ENDE *}
                                                        {/if}
                                                        {if !empty($cBeschreibung)}
{* Beschreibung entfernen START *}
                                                            {link title=$cBeschreibung|escape:"html" data=["toggle"=>"tooltip", "html"=>"true"]}
                                                                <span class="d-none"><i class="fas fa-question-circle"></i></span>
                                                            {/link}
{* Beschreibung entfernen ENDE *}
                                                        {/if}
                                                    </p>

                                                    {if $oItem->getMin() == $oItem->getMax()}
{* Quantitiy entfernen START *}
                                                        <span class="d-none">{lang key='quantity'}: {$oItem->getInitial()}</span>
{* Quantitiy entfernen ENDE *}
                                                    {else}
                        ...
                        {/if}
                        {/row}
{* WSC schliessen von oben START *}
{/col}
</div>
{* WSC schliessen von oben ENDE *}
                    {/block}
                        <div class="sticky-bottom">
                            {if $configGroup@last}
                                {nav}
{* WSC Button als SUCCESS + LG START *}
                                    {navitem id="cfg-tab-summary-finish"
                                        ...
                                        router-class="btn btn-success btn-lg"
                                        disabled=true
                                    }
                                        {lang key='finishConfiguration' section='productDetails'}
                                    {/navitem}
{* WSC Button als SUCCESS ENDE*}
                                {/nav}
                            {else}
{* WSC Button als SUCCESS + LG START *}
                                {button
